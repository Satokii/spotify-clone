...
function App() {
  ...
  const renderTopTracks = () => {
    return topTracks.map(track => (
        <div key={track.id}>
          <p>Song: {track.name} by: {track.artists[0].name}</p>
        </div>
    ))
  }
  ...
  const calcTimesPlayed = () => {
    ...
    recentlyPlayed.forEach(track => {
      const checkDups = newArr.find(updatedTrack => updatedTrack.track.id === track.track.id)
      if (checkDups) {
  ...
  const renderPlayCount = () => {
    return playCount.map(track => (
      <div key={track.id}>
        <p>Song: {track.track.name}, times played: {track.timesPlayed}, time spent listening: {new Date((track.timesPlayed * track.track.duration_ms)).toISOString().substring(11, 19)}</p>
      </div>
    ))
  }
